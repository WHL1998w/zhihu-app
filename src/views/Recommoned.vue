<template>
	<div>
		<div v-for="(item, index) in recommenedList" :key="index" class="bl-card p-3 fill-white tiny-round  border-bottom">
			...
			<div class="d-flex p-2" v-if="item.target.thumbnail">
				<div class="bl-fruid-col-3 mr-3"><img :src="item.target.thumbnail" class="fill tiny-round" /></div>
				<p class="bl-fruid-col-9 ellipsis-4">{{ item.target.author.name }}:{{ item.target.excerpt }}</p>
			</div>
			<div class="d-flex p-2 mb-4" v-else>
				<p class="bl-fruid-col-12 ellipsis">{{ item.target.author.name }}:{{ item.target.excerpt }}</p>
			</div>
