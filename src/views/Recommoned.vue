<template>
	<div>
		<div v-for="(item, index) in recommenedList" :key="index" class="bl-card  fill-white tiny-round  border-bottom">
			<div class="content-wrapper p-2">
				<!-- 获取标题的-->
				<h3 v-if="item.target.question == null" class="m-2 bl-font-weight">{{ item.target.title }}</h3>
				<h3 v-else class="m-2 bl-font-weight">{{ item.target.question.title }}</h3>
				<!-- 缩略形式显示，flag为false-->
	...
	created() {
		this.axios.get('/api/recommend').then(res => {
			...
			for (var i = 0; i < this.recommenedList.length; i++) {
				...
				//将富文本中的图片处理大小
				this.recommenedList[i].target.content = this.recommenedList[i].target.content.replace(regex, `<img style="max-width: 100%; height:40%"`);
			}
		});
	},
	methods: {
